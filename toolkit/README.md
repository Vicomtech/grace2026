...
The splitter and source unifier use only the Python standard library. The CasiMedicos preprocessing module keeps the original `pandas`-based alignment code. The scorer uses `scikit-learn` when it is installed; a small fallback is included so the tiny examples can run in minimal environments.
...
src/casimedicos-preprocessing/
                         Align CasiMedicos relations and generate split files.
...
The CasiMedicos preprocessing module is documented in `src/casimedicos-preprocessing/README.md`.
...
- CasiMedicos preprocessing should be run before source unification only when regenerating `casimedicos-dataset/raw/relations` or `casimedicos-dataset/splits`.
